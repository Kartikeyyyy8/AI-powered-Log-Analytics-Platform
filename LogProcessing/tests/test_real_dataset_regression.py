...
class RealDatasetRegressionTests(unittest.TestCase):
    ...
    def test_real_healthapp_dataset_guarantees(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("corrupted timestamp field", dead_record["error_reason"])
            self.assertIn("\x00", dead_record["raw_message"])
            self.assertIn("contains_null_byte", dead_record["quality_flags"])
            self.assertIn("corrupted_timestamp", dead_record["quality_flags"])
            ...
            # Line 1794 is at index 1793 in the valid records list because all previous records are valid.
            ...
            self.assertEqual(rec_1794["parsed_message_type"], "tryToReloadTodayBasicSteps")
